...
def training_episode(Game):
    ...
    def run(self,attack_manager,defense_manager):
        '''
        most of code essentially copied, just add training manager
        '''
        ...
        # attack_manager=train_manager(_at,n_replay=10000,batch_size=30,update_frequency=600,device="cuda",model=dqn,model_args={"n_in":20,"replay_buffer":None,"lrate":5e-4})
        # defense_manager=train_manager(_dt,n_replay=10000,batch_size=30,update_frequency=600,device="cuda",model=dqn,model_args={"n_in":20,"replay_buffer":None,"lrate":5e-4})
    
        attack_manager.reset_state_logs()
        defense_manager.reset_state_logs()
        ...
        while not self.gameOver:
            ...
            if len(attack_manager.replay_buffer)>0:
                attack_manager.training_step()
            if len(defense_manager.replay_buffer)>0:
                attack_manager.training_step()
